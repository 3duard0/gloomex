defmodule Gloomex.GloomexTest do
  ...
  test "check false positive ratio (generative test)" do
    ...
    bloom = Enum.reduce(1..initial_amount, bloom, fn x, bloom ->
      Gloomex.put(bloom, Integer.to_string(x) |> IO.inspect())
    end)

    fp = Enum.reduce((initial_amount + 1)..(initial_amount + generate_amount), 0, fn x, fp ->
      if Gloomex.might_contain?(bloom, Integer.to_string(x) |> IO.inspect()) |> IO.inspect() do
        fp + 1
      else
        fp
      end
